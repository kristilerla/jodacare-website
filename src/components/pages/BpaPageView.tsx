...
import { getBpaContent } from '@/i18n/messages/bpa-page';
import { withLocale } from '@/lib/i18n/paths';
import type { Locale } from '@/lib/i18n/types';

const userGroupIcons = [UserIcon, UsersIcon, ClipboardDocumentListIcon] as const;
const featureIcons = [
  DevicePhoneMobileIcon,
  ClipboardDocumentCheckIcon,
  ShieldCheckIcon,
...
const relatedIcons = [HomeModernIcon, ClipboardDocumentListIcon, HeartIcon] as const;

type Props = { locale: Locale };

export function BpaPageView({ locale }: Props) {
  const d = getBpaContent(locale);
  ...
  return (
    <>
      ...
      <section className="py-20 lg:py-28" aria-label={d.userGroups[0]?.title}>
        <Container>
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {d.userGroups.map((item, i) => {
              const Icon = userGroupIcons[i] ?? UserIcon;
              return (
        ...
        <Container>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {d.features.map((feature, i) => {
              const Icon = featureIcons[i] ?? DevicePhoneMobileIcon;
              return (
        ...
        <Container size="md">
          <FadeIn>
            <div className="text-center">
              <h2 id="reference-title" className="font-serif text-3xl sm:text-4xl font-bold text-text mb-6">
                {d.referenceTitle}
              </h2>
              <p className="text-lg text-text-light max-w-xl mx-auto">{d.referenceBody}</p>
            </div>
          </FadeIn>
        </Container>
      </section>
