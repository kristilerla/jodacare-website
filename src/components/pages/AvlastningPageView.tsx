...
import { getAvlastningContent } from '@/i18n/messages/avlastning-page';
import { withLocale } from '@/lib/i18n/paths';
import type { Locale } from '@/lib/i18n/types';

const perspectiveIcons = [HeartIcon, HomeIcon, ClipboardDocumentListIcon, ChartBarIcon] as const;
const featureIcons = [ArrowPathIcon, DevicePhoneMobileIcon, ShieldCheckIcon, ExclamationTriangleIcon] as const;
const relatedIcons = [HeartIcon, ClipboardDocumentListIcon, HomeIcon] as const;

type Props = { locale: Locale };

export function AvlastningPageView({ locale }: Props) {
  const d = getAvlastningContent(locale);
        ...
        <Container>
          <FadeIn>
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 id="perspectives-title" className="font-serif text-3xl sm:text-4xl font-bold text-text">
                {d.perspectivesTitle}
              </h2>
            </div>
          </FadeIn>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {d.perspectives.map((item, i) => {
              const Icon = perspectiveIcons[i] ?? HeartIcon;
              return (
        ...
        <Container>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {d.features.map((feature, i) => {
              const Icon = featureIcons[i] ?? ArrowPathIcon;
              return (
